...
def pseudorand(fun):
    ...
    If the object's __deterministic__ attribute is set to True, the random number generator should
    be seeded with a fixed value (here, 100, chosen arbitrarily) before calling the method.
    Hence the method should behave in a deterministic way, only if the instance was initialized
    with the __deterministic__ flag.
    This is essentially useful for testing.

    The decorator should raise an error if the object does not have a __deterministic__ attribute.
    """
    @functools.wraps(fun)
