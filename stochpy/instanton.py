...
class InstantonSolver:
    ...
    def instanton_fvp(self, x1, p1, *args, **kwargs):
        """
        Solve the instanton equations as differential equations, but imposing the final condition.
        x1 and p1 are the final conditions.
        args is a sequence of time points for which to compute the position and momentum.

        Return the instanton trajectory (t, x, p).
        """
        # backwardInstantonSolver = copy.deepcopy(self)
        # backwardInstantonSolver.instanton_eq = lambda t, Y: -self.instanton_eq(-t, Y)
        # backwardInstantonSolver.instanton_jac = lambda t, Y: -self.instanton_jac(-t, Y)
        # return backwardInstantonSolver.instanton_ivp(x1, p1, *args, **kwargs)
